...
static int mtk_open(struct net_device *dev)
{
	...
	if (unlikely(!is_valid_ether_addr(dev->perm_addr))) {
		of_get_ethdev_address(mac->of_node, dev);
	}
...
static int mtk_add_mux_channel(struct mtk_mux *mux, struct device_node *np)
{
	...
	/* phylink create */
	if (of_get_phy_mode(np, &phy_mode) < 0) {
...
static int mtk_add_mac(struct mtk_eth *eth, struct device_node *np)
{
	...
	if (mac->type == MTK_XGDM_TYPE) {
        ...
        if (MTK_HAS_CAPS(eth->soc->caps, MTK_2P5GPHY)) {
	...
	fixed_node = fwnode_get_named_child_node(of_fwnode_handle(mac->of_node),
						 "fixed-link");
	if (fixed_node) {
		desc = fwnode_gpiod_get_index(fixed_node, "link-gpio",
					      0, GPIOD_IN, "?");
		if (!IS_ERR(desc)) {
			...
			if (irq > 0) {
				return (devm_request_irq(eth->dev, irq, mtk_handle_irq_fixed_link,
...
static int mtk_probe(struct platform_device *pdev)
{
	...
	if (MTK_HAS_CAPS(eth->soc->caps, MTK_RSS)) {
		for (i = 0; i < MTK_RX_RSS_NUM; i++)
			netif_napi_add_weight(&eth->dummy_dev, &eth->rx_napi[MTK_RSS_RING(i)].napi,
				       mtk_napi_rx, MTK_NAPI_WEIGHT);
	}

	if (MTK_HAS_CAPS(eth->soc->caps, MTK_HWLRO)) {
		for (i = 0; i < MTK_HW_LRO_RING_NUM; i++) {
			netif_napi_add_weight(&eth->dummy_dev, &eth->rx_napi[MTK_HW_LRO_RING(i)].napi,
				       mtk_napi_rx, MTK_NAPI_WEIGHT);
		}
	}
...
static const struct mtk_soc_data mt7621_data = {
	...
	.txrx = {
		.txd_size = sizeof(struct mtk_tx_dma),
		.rx_dma_l4_valid = RX_DMA_L4_VALID,
		.tx_dma_size = MTK_DMA_SIZE(512),
		.rx_dma_size = MTK_DMA_SIZE(512),
