...
mtk_wed_hwrro_init(struct mtk_wed_device *dev)
{
	...
	if (mtk_wed_is_v3_1(dev->hw)) {
		wed_set(dev, MTK_WED_RRO_PG_BM_RX_DMAM,
			FIELD_PREP(MTK_WED_RRO_PG_BM_RX_BASE_PTR_H,
				   dev->hw_rro.desc_phys >> 32));
...
mtk_wed_rx_buffer_hw_init(struct mtk_wed_device *dev)
{
	...
	if (mtk_wed_is_v3_1(dev->hw)) {
		wed_set(dev, MTK_WED_RX_BM_RX_DMAD,
			FIELD_PREP(MTK_WED_RX_BM_RX_DMAD_BASE_PTR_H,
				   dev->rx_buf_ring.desc_phys >> 32));
...
mtk_wed_rro_rx_ring_setup(struct mtk_wed_device *dev, int idx, void __iomem *regs)
{
	...
	if (mtk_wed_is_v3_1(dev->hw))
		wed_set(dev, MTK_WED_RRO_RX_D_RX(idx) + MTK_WED_RING_OFS_COUNT,
			FIELD_PREP(MTK_WED_RRO_RX_D_RX_BASE_PTR_H,
				   readl(regs) >> 32));
...
mtk_wed_msdu_pg_rx_ring_setup(struct mtk_wed_device *dev, int idx, void __iomem *regs)
{
	...
	wed_w32(dev, MTK_WED_RRO_MSDU_PG_CTRL0(idx) + MTK_WED_RING_OFS_COUNT,
		readl(regs + MTK_WED_RING_OFS_COUNT));
	if (mtk_wed_is_v3_1(dev->hw))
		wed_set(dev, MTK_WED_RRO_MSDU_PG_CTRL0(idx) + MTK_WED_RING_OFS_COUNT,
			FIELD_PREP(MTK_WED_RRO_MSDU_PG_BASE_PTR_H,
				   readl(regs) >> 32));
...
mtk_wed_ind_rx_ring_setup(struct mtk_wed_device *dev, void __iomem *regs)
{
	...
	if (mtk_wed_is_v3_1(dev->hw))
		wed_set(dev, MTK_WED_IND_CMD_RX_CTRL1 + MTK_WED_RING_OFS_COUNT,
			FIELD_PREP(MTK_WED_IND_CMD_BASE_M,
				   readl(regs) >> 32));
	/* ack sn cr */
	...
	if (mtk_wed_is_v3_1(dev->hw))
		wed_set(dev, MTK_WED_RRO_CFG1,
			FIELD_PREP(MTK_WED_RRO_CFG1_ACK_SN_BASE_M,
				   dev->wlan.phy_base >> 32));
	...
	if (mtk_wed_is_v3_1(dev->hw))
		wed_set(dev, MTK_WED_ADDR_ELEM_CFG1,
			FIELD_PREP(MTK_WED_ADDR_ELEM_PARTICL_SE_ID_BASE_M,
				   dev->wlan.ind_cmd.particular_se_phys >> 32));
...
mtk_wed_rro_3_1_rx_ring_setup(struct mtk_wed_device *dev, void __iomem *regs)
{
	...
	wed_w32(dev, MTK_WED_WPDMA_RRO3_1_RX_D_RX + MTK_WED_RING_OFS_COUNT,
		readl(regs + MTK_WED_RING_OFS_COUNT) |
		FIELD_PREP(MTK_WED_WPDMA_RRO3_1_RX_D_RX_BASE_PTR_H,
			   readl(regs) >> 32));
	...
	__releases(RCU)
{
	...
	dev_info(device, "attaching wed device %d version %d.%d\n",
		 hw->index,
		...
		void __iomem *regs, bool reset)
{
	...
	if (mtk_wed_is_v3_1(dev->hw)) {
		wpdma_tx_w32(dev, idx, MTK_WED_RING_OFS_COUNT,
			     MTK_WED_TX_RING_SIZE |
			     FIELD_PREP(MTK_WED_WPDMA_RING_TX_BASE_PTR_H,
					ring->desc_phys >> 32));
		wed_set(dev, MTK_WED_WPDMA_RING_TX(idx) + MTK_WED_RING_OFS_COUNT,
			FIELD_PREP(MTK_WED_WPDMA_RING_TX_BASE_PTR_H,
				   ring->desc_phys >> 32));
	}
	return 0;
